...
namespace CitadellesV1
{
    ...
    public partial class Form1 : Form
    {
       ...
       Form FormChoixPioche = new Form();
                ...
                CliqueConstrruction++;
                    ...
                    if (nombreCarteQuartier != 0)
                    {
                        ...
                        foreach (PictureBox carteQuartierJoueur in ListeDesQuartiers)
                        {
                            ...
                            Quartier.BackColor = carteQuartierJoueur.BackColor;
                            Quartier.Size = carteQuartierJoueur.Size;
                            Quartier.SizeMode = carteQuartierJoueur.SizeMode;
                            Quartier.Name = carteQuartierJoueur.Name;
                            Quartier.Tag = carteQuartierJoueur.Tag;
                            afficheCarteQuartier.Controls.Add(Quartier);
                            listeIndex += afficheCarteQuartier.Controls.IndexOf(Quartier); 
                            Quartier.Top = 30;
        ...
        private void tousLesControls(Control PaquetCarte, List<PictureBox> toutesLesCartes)
        {
            ...
            foreach(PictureBox controle in PaquetCarte.Controls)
            {
               // tousLesControls(controle, toutesLesCartes);
                ListeDesQuartiers.Add(controle);
            }
        ...
        private void MiseAJourQuartiers(TabPage plop , List<PictureBox>Lesquartiers)
        {
            int cartes = 0;
            foreach (PictureBox machin in Lesquartiers)
            {
                
                    cartes = Compte_carte(plop);
                    if (cartes <= 4)
                    {
                        machin.Top = cartes * 150;
                        machin.Left = 0;
                    }
                    else if (cartes > 4)
                    {
                        machin.Top = (cartes - 4) * 150;
                        machin.Left = 90;
                    }
                    plop.Controls.Add(machin);
            }
        }

        private int MiseAJourListeQuartiers(String Nomquartier, List<PictureBox>Lesquartiers)
        {
            bool dejaEnleve = false;
            int enlever=0;
            if(Lesquartiers.Count != 1)
            {
                foreach (PictureBox plop in Lesquartiers)
                {
                    if (plop.Name == Nomquartier && !dejaEnleve)
                    {
                        enlever = Lesquartiers.IndexOf(plop);
                        dejaEnleve = true;
                    }
                }
            }
        ...
        private void ChoixConstructionQuartier(object sender, EventArgs e)
        {
            ...
            String LeNomQuartier = QuartierSelectionné.Name;
            ...
            if (nombre_de_pieces >= prixQuartierSelectionné)
            {
                ...
                if (Numero_Joueur_Tour == 1)
                {
                    int essaiplop = MiseAJourListeQuartiers(LeNomQuartier, quartiersJ1);
                    if(essaiplop == 0)
                    {
                        quartiersJ1.Clear();
                    }
                    ...
                    {
                        quartiersJ1.RemoveAt(essaiplop);
                    }
                    
                    tabPage2.Controls.Clear();
                    MiseAJourQuartiers(tabPage2, quartiersJ1);

                }
                else
                {
                    int essaiplop = MiseAJourListeQuartiers(LeNomQuartier, quartiersJ2);
                    if (essaiplop == 0)
                    {
                        quartiersJ1.Clear();
                    }
                    ...
                    {
                        quartiersJ2.RemoveAt(essaiplop);
                    }
                   
                    tabPage4.Controls.Clear();
                    MiseAJourQuartiers(tabPage4, quartiersJ2);
                }
